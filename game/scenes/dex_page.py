...
class CardDisplay:
    ...
    def __init__(self, card: Card, card_manager, collection_data: Dict[str, Any] = None, game_manager=None):
        self.card = card
        self.card_manager = card_manager  # card_manager引用
        self.game_manager = game_manager  # game_manager引用
        self.collection_data = collection_data or {}
        
        # # 🆕 添加调试信息
        # print(f"🎨 创建CardDisplay: {card.name}")
        # print(f"🔍 CardDisplay接收到的game_manager: {type(self.game_manager)}")
        # print(f"🔍 card_manager有game_manager: {hasattr(self.card_manager, 'game_manager')}")
    ...
    def _create_static_surfaces(self):
        ...
        if hasattr(self.card, 'id') and self.card.id:
            ...
            if image_path and os.path.exists(image_path):
                # 优先使用缓存系统
                if self.game_manager and hasattr(self.game_manager, 'get_cached_image'):
                    cached_image = self.game_manager.get_cached_image(image_path)
                    if cached_image:
                        img_width = self.width - 8
                        img_height = int(img_width * 330 / 240)
                        self.card_image = pygame.transform.smoothscale(cached_image, (img_width, img_height))
                        # print(f"✅ 缓存加载: {self.card.name}")  # 可选：保留简单日志
                    else:
                        print(f"❌ 缓存失败: {self.card.name}")
                
                # 🆕 保留备用方案（但通常不会执行）
                if self.card_image is None:
                    print(f"⚠️ 使用备用加载: {self.card.name}")
                    try:
...
class DexPage:
    ...
    def __init__(self, screen_width: int, screen_height: int, ui_manager, card_manager, game_manager, nav_bar_height: int = 90):
        ...
        # 🆕 验证传入的card_manager
        print(f"🔍 DexPage接收到的CardManager有game_manager: {hasattr(self.card_manager, 'game_manager')}")
        print(f"🔍 DexPage接收到的CardManager类型: {type(self.card_manager)}")
        print(f"🔍 DexPage接收到的GameManager类型: {type(self.game_manager)}")

        # 从game_manager获取db_manager（更好的方式）
        self.db_manager = game_manager.db_manager if hasattr(game_manager, 'db_manager') else None
        ...
        if self.game_manager:
            self._cached_version = self.game_manager.get_card_cache_version()
            print(f"🔄 初始化DexPage，同步缓存版本: v{self._cached_version}")
        else:
            self._cached_version = 0
        self._card_displays_pool = {}  # CardDisplay对象池
    ...
    #     try:
    #         if self.game_manager:
    #             collection_stats = self.game_manager.get_user_collection_stats()
    #             user_cards = self.game_manager.get_user_cards()

    #             print("🔍 collection_stats 内容:", collection_stats)
    #             print("🔍 collection_stats 的键:", list(collection_stats.keys()))
    #             print("🔍 user_cards 内容:", user_cards)
    #             print("🔍 user_cards 的键:", list(user_cards[0].keys()) if user_cards else [])
                
    #             # 转换为你需要的格式
    #             self.user_collection = {}
    #             for card_info in user_cards:
    #                 # 修复：使用点号访问Card对象的属性
    #                 card_id = card_info['card'].id  # 或者可能是 .card_id
    #                 self.user_collection[card_id] = {
    #                     'quantity': card_info['quantity'],
    #                     'obtained_at': card_info['obtained_at']
    #                 }
                
    #             # 使用gamemanager计算的统计数据
    #             self.owned_cards = collection_stats['owned_stats']['unique_cards_owned']
    #             self.total_cards = collection_stats['total_cards']
    #             self.completion_rate = collection_stats['completion_percentage']
                
    #             print(f"用户收集: {self.owned_cards}/{self.total_cards} ({self.completion_rate:.1f}%)")
    #         else:
    #             self._reset_collection_data()
    #     except Exception as e:
    #         print(f"❌ 加载用户收集数据失败: {e}")
    #         self._reset_collection_data()

    # def _reset_collection_data(self):
    #     """重置收集数据"""
    #     self.user_collection = {}
    #     self.owned_cards = 0
    #     self.completion_rate = 0.0
    ...
    def _load_user_collection(self):
        ...
        try:
            if self.game_manager:
                ...
                # 使用正确的键名访问统计数据
                self.owned_cards = collection_stats['total_progress']['collected']  # 修复：使用correct key
                self.total_cards = collection_stats['total_progress']['total']      # 修复：使用correct key  
                self.completion_rate = collection_stats['total_progress']['percentage']  # 修复：使用correct key
    ...
    def _reset_collection_data(self):
        ...
        self.owned_cards = 0
        self.completion_rate = 0.0
        self.total_cards = len(self.all_cards) if hasattr(self, 'all_cards') else 0
    ...
    def _create_card_displays(self):
        """创建卡牌显示组件（使用对象池优化）"""
        print(f"🏭 开始创建CardDisplay对象，总数: {len(self.filtered_cards)}")
        self.card_displays = []
        
        for card in self.filtered_cards:
            card_id = getattr(card, 'id', '')
            collection_data = self.user_collection.get(card_id, {})
            
            # 使用对象池避免重复创建
            if card_id not in self._card_displays_pool:
                # print(f"🆕 创建新的CardDisplay: {card.name} (ID: {card_id})")
                self._card_displays_pool[card_id] = CardDisplay(
                    card, 
                    self.card_manager, 
                    collection_data,
                    game_manager=self.game_manager  # 🆕 确保传递game_manager
                )
            else:
                print(f"♻️ 重用CardDisplay: {card.name} (ID: {card_id})")
                # 更新现有对象的收集数据
                display = self._card_displays_pool[card_id]
                display.collection_data = collection_data
                display.quantity = collection_data.get('quantity', 0)
                display.is_owned = display.quantity > 0
                display.obtained_at = collection_data.get('obtained_at')
                display._render_text()
                display._cache_dirty = True
            
            self.card_displays.append(self._card_displays_pool[card_id])
        
        print(f"📦 CardDisplay对象池: {len(self._card_displays_pool)} 个对象")
