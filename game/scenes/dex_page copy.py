...
class CardDisplay:
    ...
    def __init__(self, card: Card, collection_data: Dict[str, Any] = None):
        self.card = card
    ...
    def _create_static_surfaces(self):
        ...
        if hasattr(self.card, 'image_path') and self.card.image_path:
            image_path = os.path.join("assets", "card_assets", self.card.image_path)
...
class DexPage:
    ...
    def __init__(self, screen_width: int, screen_height: int, ui_manager, db_manager: DatabaseManager, nav_bar_height: int = 90):
    ...
    def _load_card_data(self):
        """加载卡牌数据"""
        try:
            if self.db_manager and hasattr(self.db_manager, 'card_dao'):
                # 从数据库获取所有卡牌
                self.all_cards = self.db_manager.card_dao.search_cards(limit=10000)
                print(f"从数据库加载了 {len(self.all_cards)} 张卡牌")
            else:
                print("⚠️ 数据库管理器不可用，使用模拟数据")
                self.all_cards = []
        except Exception as e:
            print(f"❌ 加载卡牌数据失败: {e}")
            self.all_cards = []
    ...
    def _create_scrollbar(self):
        ...
        if content_height > visible_height:
            ...
            # 创建滚动条
            scrollbar_height = visible_height - 20
            self.ui_elements['scrollbar'] = UIVerticalScrollBar(
                relative_rect=pygame.Rect(self.screen_width - 25, self.content_start_y + 10, 15, scrollbar_height),
                visible_percentage=min(1.0, visible_height / content_height),
                manager=self.ui_manager,
                object_id=ObjectID('#dex_scrollbar')
            )
    ...
    def _setup_ui_theme(self):
        ...
        theme_data = {
            ...
            '#effect_button_active': {
                'colours': {
                    'normal_bg': '#10B981',
                    'hovered_bg': '#059669',
                    'normal_border': '#047857',
                    'normal_text': '#FFFFFF'
                },
                'misc': {
                    'border_radius': '6',
            ...
            '#dex_scrollbar': {
                'colours': {
                    ...
                    'normal_button': '#CBD5E1',
                    'hovered_button': '#94A3B8'
                },
                'misc': {
                    'border_radius': '8',
                    'border_width': '0'
    ...
    def _get_all_rarities(self) -> List[str]:
        ...
        try:
            if self.db_manager and hasattr(self.db_manager, 'card_dao'):
                return self.db_manager.card_dao.get_all_rarities()
            return []
        except Exception as e:
            print(f"获取稀有度失败: {e}")
            return []
    ...
    def _get_all_types(self) -> List[str]:
        ...
        try:
            if self.db_manager and hasattr(self.db_manager, 'card_dao'):
                return self.db_manager.card_dao.get_all_types()
            return []
        except Exception as e:
            print(f"获取类型失败: {e}")
            return []
    ...
    def _create_card_displays(self):
        ...
        for card in self.filtered_cards:
            ...
            card_display = CardDisplay(card, collection_data)
