...
class BattleController:
    ...
    def start_new_battle(self, player_deck_id: int, opponent_type: str = "AI", opponent_id: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"🚀 启动战斗: 玩家ID={player_id}, 卡组ID={player_deck_id}, 对手={opponent_type}, 对手ID={opponent_id}")
            ...
            if self.current_battle.battle_state and len(self.current_battle.player_states) > 0:
                ...
                if self.current_battle.start_battle():
                    print(f"✅ 战斗启动成功，ID: {self.current_battle.battle_state.battle_id}")
                    return {
                        "success": True,
    ...
    def process_player_action(self, action_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.current_battle.battle_state.is_battle_over():
                ...
                result["result"] = self.current_battle.battle_state.result.value
    ...
    def end_battle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                "duration": self.current_battle.battle_state.get_battle_duration() if hasattr(self.current_battle.battle_state, 'get_battle_duration') else 0
            }
    ...
    def is_battle_active(self) -> bool:
        """检查是否有活跃的战斗"""
        return (self.current_battle is not None and 
                hasattr(self.current_battle, 'battle_state') and
                self.current_battle.battle_state is not None and
                not self.current_battle.battle_state.is_battle_over())
    
    def surrender_battle(self) -> Dict[str, Any]:
        """投降当前战斗"""
        if not self.current_battle:
            return {"success": False, "error": "没有活跃的战斗"}
        ...
        try:
            player_id = self.game_manager.current_user_id
            success = self.current_battle.surrender(player_id)
            
            if success:
                return {"success": True, "message": "投降成功"}
            else:
                return {"success": False, "error": "投降失败"}
                
        except Exception as e:
            print(f"❌ 投降失败: {e}")
            return {"success": False, "error": f"投降失败: {e}"}
    
    def pause_battle(self) -> Dict[str, Any]:
        """暂停战斗"""
        if not self.current_battle:
            return {"success": False, "error": "没有活跃的战斗"}
        
        try:
            self.current_battle.pause_battle()
            return {"success": True, "message": "战斗已暂停"}
        except Exception as e:
            return {"success": False, "error": f"暂停失败: {e}"}
    
    def resume_battle(self) -> Dict[str, Any]:
        """恢复战斗"""
        if not self.current_battle:
            return {"success": False, "error": "没有活跃的战斗"}
        
        try:
            self.current_battle.resume_battle()
            return {"success": True, "message": "战斗已恢复"}
        except Exception as e:
    ...
    def get_battle_log(self) -> List[str]:
        ...
        try:
            return self.current_battle.get_battle_log()
        except Exception as e:
            print(f"❌ 获取战斗日志失败: {e}")
            return []
